...
class FirSlf4jDeclarationGenerator(
    ...
    private fun shouldGenerateFor(classSymbol: FirClassSymbol<*>): Boolean {
        ...
        // Skip local/anonymous classes — captured type parameters from enclosing
        // generic methods get corrupted during IR linking (same issue as K1 path).
        if (classSymbol.classId.isLocal) return false

        // User-configured opt-out (JPA entities, etc.). Also skip companion
        // objects of excluded classes because companions don't inherit the
        // owner's annotations.
        if (excludeAnnotationSet.isNotEmpty()) {
            val classAnnotations = classSymbol.resolvedAnnotationClassIds.map { it.asSingleFqName().asString() }
            if (classAnnotations.any { it in excludeAnnotationSet }) return false
            val isCompanion = classKind == ClassKind.OBJECT &&
                classSymbol.name.asString() == "Companion" &&
                classSymbol.classId.outerClassId != null
            if (isCompanion) {
                val ownerId = classSymbol.classId.outerClassId!!
                val ownerSym = session.symbolProvider.getClassLikeSymbolByClassId(ownerId) as? FirRegularClassSymbol
                if (ownerSym != null) {
                    val ownerAnnotations = ownerSym.resolvedAnnotationClassIds.map { it.asSingleFqName().asString() }
                    if (ownerAnnotations.any { it in excludeAnnotationSet }) return false
                }
            }
        }
